...
def init_dict():
    with open('37k.txt', 'r', encoding='utf-8') as f:
        for line in f:
            line = line.rstrip('\n')
...
class ProgressManager:
    ...
    def update(self, target, progress):
        ...
        elif (current_time - self._last_active_time) > 20.0:
            self._last_active_time = current_time
        ...
        for total, idx, auth in get_authentication():
            ...
            for step in range(self.RETRY_TIMES):
                try:
                    ...
                    self._alive = True  # if request.get() fail, will raise exceptions
    ...
    def start(self):
        ...
        while True:
            if len(scheduled_tasks) > 0:
                while threading.active_count() < self._max_threads:
            ...
            print('threads: {0}, tries: {1:.2f}/s'.format(threading.active_count() - 1,
...
def main():
    manager = ThreadManager(max_threads=512)
    manager.start()
